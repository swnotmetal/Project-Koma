...
The Cloudflare Worker serves all four tabs. Gate runs the real `koma-gate`
classifier, Scout runs deterministic pre-flight checks, and Core runs the real
`koma-core` package with an in-memory demo adapter. Miko's static fixture is
generated by the real Verifier and rendered as a CLI-shaped browser simulation;
it opens first and needs no model backend. Raw event JSON and the Agent Spec stay
available in expandable details for technical inspection. The compatibility date
