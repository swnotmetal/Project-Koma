...
Scout's audio validation layer was designed from this pattern: reject before processing. Size check. Duration check. MIME check. Cooldown enforcement. If the audio can't convince a deterministic perimeter check, it never reaches the LLM.
...
Gate was built to draw that line. Before any model call, a lightweight classifier answers one question: is this input inside the scope of what this assistant is supposed to handle? The model never sees the rejected requests. Four presets cover the most common AI app boundaries: general knowledge, code assistant, customer support, and factual reference tools.
...
Core separates the index from the content. Search returns lightweight metadata — a title, a category, an opaque token. The actual payload lives in a separate collection, addressed only by that token. Tokens are HKDF-derived from a backend secret. Clients can't guess them. Even if search is wide open, the data behind it isn't.
